...
export default function ChannelBar({channels, serverName, setChannels, setMessageList, loadMessages, setChannelName, serverID}) {
    ...
    return (
        <div className="min-h-0 w-60 bg-[#2f3136] grow-0 shrink-0 basis-auto max-h-full">
            ...
            <div className={`${showDropdown ? "visible" : "invisible"}`}>
                <ChannelDropDown setShowDropdown={setShowDropdown} setChannels={setChannels} channels={channels} serverID={serverID}/>
            </div>
            <div className="h-[calc(100%_-_100px)] flex-1 grow-1 basis-auto">
                <ul className="h-full max-h-[calc(100%_-_100px)]">
                    <div className='p-4' key="servers">
                        {channels.map((info) => 
                            <ChannelButton channelID={info.channelID} name={info.name} key={info.pos_order} setMessageList={setMessageList} loadMessages={loadMessages} setChannelName={setChannelName}/>
                        )}
                    </div>
                </ul>
            </div>
